...
var DashboardForm = function () {
    function DashboardForm() {
        ...
        this.btnBlueCardRegister = $('#btnBlueCardRegister');
        this.btnConfirmBlueCardRegister = $('#btnConfirmBlueCardRegister');
        this.btnGetPoint = $('#btnGetPoint');
        this.btnBluecard = $('#btnBluecard');
        this.bluecardModal = $('#bluecard-modal');
        this.bluecard = $('#bluecard');
        this.blueCardRedeemModal = $('#blueCardRedeemModal');

        this.hidwifi_brand = $('#WifiModel_wifi_brand');
        this.hidwifi_branch = $('#WifiModel_wifi_branch');
        this.hidwifi_pos = $('#WifiModel_wifi_pos');
    ...
    _createClass(DashboardForm, [{
        ...
        value: function init() {
            ...
            this.txtValueSearch.keyup(function (e) {
            ...
            this.btnBlueCardRegister.click(function () {

                if (me.bluecardModal.attr('class') === "modal right fade show") {
                    me.bluecardModal.show();

                } else {
                    me.bluecardModal.modal();
                }
                //   $('#btnConfirmBlueCardRegister').prop("disabled", true);
            });
            this.btnBluecard.click(function () {

                me.bluecard.modal();
                //   $('#btnConfirmBlueCardRegister').prop("disabled", true);
            });

            this.btnGetPoint.click(function () {

                //  document.getElementById("getPointBlueCard").reset();
                $('input').val('');
                if (me.blueCardRedeemModal.attr('class') === "modal fade show") {
                    me.blueCardRedeemModal.show();

                } else {
                    me.blueCardRedeemModal.modal();
            ...
            $('#btngenwifi').click(function () {
                $.ajax({
                    type: "post",
                    url: "Scripts/hash_function.php",
                    data: {
                        action: 'encrypt',
                        brand: me.hidwifi_brand.val(),
                        branch: me.hidwifi_branch.val(),
                        pos: me.hidwifi_pos.val()
                    },
                    success: function (hashd) {
                        //var data = {
                        //    hashpwd: result
                        //};"{\"encrypt\":\"AN2262XT23X\"}"
                        $.post(me.getWifiGen,
        ...
        value: function searchVehicleCustomer() {
            ...
            var wait = setTimeout(function () {
                return me.dt.ajax.reload(function (json) {
                    if (json) console.log(json);
                    if ($('#search-list tbody tr')[0].innerText != 'ไม่พบข้อมูลที่ค้นหา' && $('#search-list tbody tr').length == 1) $('#search-list tbody tr').click();
                });
            }, 500);
        ...
        value: function reload() {
            var me = this;
            if (me.dt != undefined) {
                if (me.dt.ajax.url() === me.getSearchListUrl && me.dt.data().length === 0) {
                    me.dt.ajax.url(me.getSearchListHqUrl).load();
                } else if (me.dt.ajax.url() === me.getSearchListHqUrl && me.dt.data().length === 0) {
        ...
        value: function createDatatable() {
            ...
            this.dt = this.dtSearchList.DataTable({
                ...
                "scrollCollapse": true,
                "ajax": {
                    "url": me.getSearchListUrl,//me.getSearchListHqUrl,
                    "type": "POST",
                    ...
                    error: function error(jqXHR, exception) {
                        //  debugger
                        //   alert("เชื่อมต่อ HQ ไม่ได้ ระบบกำลังดึงข้อมูลจาก POS Local");
                        setTimeout(function () {
                            return (
            ...
            $('#search-list tbody').on('click', 'tr', function () {
                if ($(this).hasClass('selected')) {
                    ...
                    $.get(me.CheckCustomerCarDataUrl, row, function (resp) {
                        if (resp.Code == "00") {
                            console.log("data retreive to client complete  ");
            ...
            //check data in client === if no have in client = create //
            ...
            $('#btn-new-deposit').click(function () {
                ...
                //$.get(me.CheckCustomerCarDataUrl, row, function (resp) {
                //    if (resp.Code == "00") {
                if (row.Chassis != null && row.CustomerCode != null) {
                    $.redirect('./Home/ClassicAsp?AspFile=rt_dp_new.asp', {
                        AspFile: 'rt_dp_new.asp?' + 'encoder=1' + '&car_chasis=' + row.Chassis + '&cus_code=' + row.CustomerCode
                    });
                }
                else {
                    $.redirect('./Home/ClassicAsp?AspFile=rt_dp_new.asp', {
            ...
            $('#btn-new-car').click(function () {
                ...
                $.get(me.CheckCustomerCarDataUrl, row, function (resp) {
                    if (resp.Code == "00") openDocFull(me.clientUrl + '/bs_custcar_client.asp?cus_code=' + row.CustomerCode + '&user_id=' + userId + '&dealercode=' + dealerCode + '&act=add&page_back=close'); else console.error(resp.Message);
                });
            });
        }
    }]);

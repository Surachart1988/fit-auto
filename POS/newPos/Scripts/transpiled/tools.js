    ...
    (function ($) {
        ...
        $.fn.val = function (value) {
            ...
            if (arguments.length >= 1) {
                ...
                if (value && $.isNumeric(value) && +value >= 1000 && inputName.indexOf(this.attr('id')) > -1) value = value.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
...
function confirmAlert(massage, functional) {
    swal(_defineProperty({
        ...
        icon: '../Content/img/ICON/warning-icon.jpg',
        buttons: true
    }, 'buttons', ['ตกลง', 'ยกเลิก'])).then(functional);
}
...
function confirmAlertForProductSet(massage, functional) {
    swal({
        //title: 'คุณแน่ใจที่จะดำเนินการต่อหรือไม่ ? ',
        ...
        //dangerMode: true,
        buttons: [
            'ยืนยันการเลือกสินค้าเฉพาะรายการ',
            'กลับไปเลือกสินค้าแบบชุด'
        ],
        allowOutsideClick: false,
        closeOnClickOutside: false

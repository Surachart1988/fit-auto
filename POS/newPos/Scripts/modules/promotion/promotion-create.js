...
function CheckData() {
    $("#btnNextStep2").click(function (e) {
        ...
        let _error = 0;
        $("#pro_name").val().length === 0 ? _error++ : 0;
        $("#pro_start_date").val().length === 0 ? _error++ : 0;
        $("#pro_end_date").val().length === 0 ? _error++ : 0;
        $("#campaign_id  option:selected").val().length === 0 ? _error++ : 0;
        $("input:checkbox[name^='Dealers']:checked").length === 0 ? _error++ : 0;

        if (_error > 0) {
            e.preventDefault();
            warningAlert("กรุณากรอกข้อมูลให้ครบ");
        }
    });
